...
def generate_oblivious_tm(max_in, max_eg, basic_traffic_matrix, num_switch, num_tm):
    ...
    while(m < num_tm):
        ...
        for s in range(num_switch):
            for d in range(num_switch):
                ...
                if sum_ingress[s] + tmp_tm[s][d] > max_in[s]:
                    tmp_tm[s][d] = max_in[s] - sum_ingress[s]
                if sum_egress[d] + tmp_tm[s][d] > max_eg[d]:
                    tmp_tm[s][d] = max_eg[d] - sum_egress[d]
...
def main():
    ...
    new_tm = generate_oblivious_tm(max_ingress, max_egress, basic_traffic_matrix, num_switch, num_random_tm)

    output_file = project_path + "/traffic/10oblivious"
